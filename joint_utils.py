...
def custom_output(emb, gemma):
    ...
    class_labels = [101, 201, 301, 401, 501, 601, 701, 801, 901, 1001, 1101, 1201] # 956, 3276
...
def custom_bce_loss(binary_logits, ternary_logits, labels, loss_fns):
    ...
    #avg_bce = sum(losses) / len(losses) # Loss to use when we average over all tasks
    losses.sort(reverse=True)
    bce = sum(losses[0:2]) / 3 # Loss to use when we focus on the worst classes

    return bce

def binary_bce_loss(logits, labels, loss_fns):
    losses = []
...
def train_epoch(models, optimizers, mse_loss, loss_fns, train_loader, device, gemma, beta):
    ...
    for batch_index, (x, y) in enumerate(train_loader, 1):
        ...
        loss_bce = binary_bce_loss(logits, labels.float(), loss_fns)
...
def validate(models, mse_loss, loss_fns, val_loader, device, gemma, beta):
    ...
    with torch.no_grad():
        for batch_index, (x, y) in enumerate(val_loader, 1):
            ...
            #print('inputlength',len(vd_inputs))
            #print('labelslength',len(labels))
...
def training_loop(models, optimizers, mse_loss, loss_fns, train_loader, val_loader, num_epochs, gemma, beta):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        #folder = 'multiresult/joint_assymetric'
        ...
        with open(f"{folder}/val_accs.pkl", 'wb') as f3:
